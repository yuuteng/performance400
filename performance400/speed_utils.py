...
def get_corrected_speed_profiles(trajectory, refresh_rate, windows_size_for_average=6, savgol1=13, savgol2=7, medfilt=11):
    """

    Calculate the corrected speed profiles
    ...
    norm_speed_XY = [m.sqrt(speedX[i] ** 2 + speedY[i] ** 2) for i in range(len(speedY))]
    for i in range(norm_speed_XY):
        if(norm_speed_XY[i]>15):
            del norm_speed_XY[i]
            del index_speed[i]
    norm_speed_XY_SavFil = scipy.signal.savgol_filter(norm_speed_XY, savgol1, savgol2)
    norm_speed_XY_Medfilt = scipy.signal.medfilt(norm_speed_XY, medfilt)

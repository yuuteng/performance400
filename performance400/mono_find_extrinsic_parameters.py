...
img_points = [[1572, 1534], [1640, 1557], [1708, 1578], [1780, 1601], [1853, 1628], [1925, 1651], [1997, 1674],
              [2036, 768],
...
np.savetxt('matrices/obj_points_testMono1', obj_points)
np.savetxt('matrices/img_points_testMono1', img_points)
...
distance_coefficients = np.loadtxt('matrices/distortion_vector')

retval, camera_matrix, distance_coefficients, rvecs, tvecs = cv2.calibrateCamera([obj_points], [img_points], size,
                                                                                 camera_matrix,
...
np.savetxt('matrices/adjusted_distortion_vector', distance_coefficients)
...
mean_error = 0
# dessiner les axes
frame = cv2.drawFrameAxes(frame, camera_matrix, distance_coefficients, rvecs[0], tvecs[0], 100)
cv2.imwrite('images/frame1_axes.jpg', frame)

# redessiner les points source et calculer le pourcentage d'erreur comis
img_points2, jacobian = cv2.projectPoints(obj_points, rvecs[0], tvecs[0], camera_matrix, distance_coefficients)

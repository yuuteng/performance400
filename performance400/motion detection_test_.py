    ...
    for k in range(len(m_trajectory)):
        if (m_trajectory[k] == (None, None)):
            ...
            for i in range(k - p, k + p+1):
                print(i)
                if (m_trajectory[i][1] != None):
                    m_trajectory[k] = (m_trajectory[i][0]+m_trajectory[k][0],m_trajectory[i][1]+m_trajectory[k][1])
                    Nb += 1
                    print(m_trajectory[k])
                m_trajectory[k] =( m_trajectory[k][0] / Nb,m_trajectory[k][1] / Nb)
    return m_trajectory
...
plot.xlabel("Temps")
plot.ylabel("Vitesse")
plot.plot(scipy.signal.savgol_filter(velocity,21,5))
plot.show()
